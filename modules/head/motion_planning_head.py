...
from ..cnn.transformer import gen_sineembed_for_position
from .blocks.core_blocks import *
from .blocks.motion_blocks import *
from .blocks.instance_bank import topk
from .blocks.instance_queue import *
from .blocks.target import *
from .decoder.motion_decoder import *
from ..cnn.transformer import *
...
class MotionPlanningHead(BaseModule):
    def __init__(
        ...
        self.instance_queue = build_module(instance_queue)
        self.motion_sampler = build_module(motion_sampler)
        self.planning_sampler = build_module(planning_sampler)
        self.motion_decoder = build_module(motion_decoder)
        self.planning_decoder = build_module(planning_decoder)
        self.op_config_map = {
            "temp_gnn": temp_graph_model,
            "gnn": graph_model,
            "cross_gnn": cross_graph_model,
            "norm": norm_layer,
            "ffn": ffn,
            "refine": refine_layer,
        }
        ...
        self.motion_loss_cls = build_module(motion_loss_cls)
        self.motion_loss_reg = build_module(motion_loss_reg)
        self.plan_loss_cls = build_module(plan_loss_cls)
        self.plan_loss_reg = build_module(plan_loss_reg)
        self.plan_loss_status = build_module(plan_loss_status)
