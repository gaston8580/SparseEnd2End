...
class SparseBox3DTarget(BaseTargetWithDenoising):
    ...
    def _box_cost(self, box_pred, box_target, instance_reg_weights):
        """
        Args:
            box_pred: TensorShape(bs*self.num_dn_groups, 2*max_dn_gt, 10)
            box_target: TensorShape(bs*self.num_dn_groups, max_dn_gt, 10)
            instance_reg_weights: TensorShape(bs*self.num_dn_groups, max_dn_gt, 10)

        """
        bs = box_pred.shape[0]
        cost = []
        for i in range(bs):
            ## ((64, 1, 10) - (1, 32, 10)) * (1, 32, 10) * (10, )
            ## (64, 32, 10) * (1, 32, 10) * (10, )
            ## (64, 32, 10)
            ## torch.sum(Shape(64, 32, 10), dim=-1) => (64, 32)
            if len(box_target[i]) > 0:
                cost.append(
                    ...
                    * self.box_weight  # 0.25
                )
            else:
                cost.append(None)
        return cost

    def get_dn_anchors(self, cls_target, box_target, gt_track_id=None):
        """
        Args:
            cls_target: List[Tensor]
                        Shape(n, )
            box_target: List[c]
                        Shape(n, 9)
            gt_track_id: List[Tensor]
                        Shape(n,)

        Return:
            dn_anchor : TensorShape(bs, num_dn_groups*num_gt, 10) float32
            dn_box_target: TensorShape(bs, num_dn_groups*num_gt, 10) float32
            dn_cls_target: TensorShape(bs, num_dn_groups*num_gt) int64
                            其中-3代表未匹配上的anchor
            attn_mask: TensorShape(num_dn_groups*num_gt, num_dn_groups*num_gt)  bool
                       按num_groups划分矩阵, 对角线位置设置为False
            valid_mask: TensorShape(bs, num_dn_groups*num_gt) bool
            dn_id_target: TensorShape(bs, num_dn_groups*num_gt) int64
                          其中-1代表未匹配上的anchor
        """
        if self.num_dn_groups <= 0:  # default=5
            return None
        if self.num_temp_dn_groups <= 0:  # default=3
            gt_track_id = None

        if self.max_dn_gt > 0:  # default=32
            cls_target = [x[: self.max_dn_gt] for x in cls_target]
            box_target = [x[: self.max_dn_gt] for x in box_target]
            if gt_track_id is not None:
                gt_track_id = [x[: self.max_dn_gt] for x in gt_track_id]
        ...
        ## List[TensorShape(32, 9),...] => List[TensorShape(32, 10),...]
        box_target = self.encode_reg_target(box_target, cls_target.device)
        ## shape(bs, max_dn_gt, 10) x,y,z,w,l,h,sin,cos,vx,vy
        ## 对齐过程中，缺少的部分自动补0
        ...
        ## 对齐batch内每一帧的gt_track_id长度，缺少的部分自动补-1
        ## shape(bs, max_dn_gt)
        if gt_track_id is not None:
            gt_track_id = torch.stack(
                [F.pad(x, (0, max_dn_gt - x.shape[0]), value=-1) for x in gt_track_id]
            )

        bs, num_gt, state_dims = box_target.shape
        if self.num_dn_groups > 1:
            # (bs, max_dn_gt) => (bs*self.num_dn_groups, max_dn_gt)
            cls_target = cls_target.tile(self.num_dn_groups, 1)
            # (bs, max_dn_gt, 10) => (bs*self.num_dn_groups, max_dn_gt, 10)
            box_target = box_target.tile(self.num_dn_groups, 1, 1)
            if gt_track_id is not None:
                gt_track_id = gt_track_id.tile(self.num_dn_groups, 1)

        # 产生 [-1,1） 随机数
        noise = torch.rand_like(box_target) * 2 - 1
        # xyz噪声扰动范围 [-2, 2） wlhyawv [-0.5, 0.5）
        noise *= box_target.new_tensor(self.dn_noise_scale)
        dn_anchor = box_target + noise
        if self.add_neg_dn:
            # 产生 [1, 2） 随机数
            noise_neg = torch.rand_like(box_target) + 1
            # 均匀的产生一半mask 1, 一半 mask -1
            flag = torch.where(
            ...
            # 产生 (-2, 2） 噪声扰动
            noise_neg *= flag
            # xyz噪声扰动范围 [-4, 4） wlhyawv [-1, 1）
            noise_neg *= box_target.new_tensor(self.dn_noise_scale)
            # (bs*self.num_dn_groups, max_dn_gt, 10) => (bs*self.num_dn_groups, max_dn_gt*2, 10)
            dn_anchor = torch.cat([dn_anchor, box_target + noise_neg], dim=1)
            num_gt *= 2

        # return list[TensorShape(max_dn_gt*2, max_dn_gt),..] length=bs*num_dn_groups
        box_cost = self._box_cost(dn_anchor, box_target, torch.ones_like(box_target))
        dn_box_target = torch.zeros_like(
            dn_anchor
        )  # (bs*num_dn_groups, max_dn_gt*2 ,10)
        dn_cls_target = (
            -torch.ones_like(cls_target) * 3
        )  # (bs*num_dn_groups, max_dn_gt)
        if gt_track_id is not None:
            dn_id_target = -torch.ones_like(
                gt_track_id
            )  # (bs*num_dn_groups, max_dn_gt)
        if self.add_neg_dn:
            # (bs*num_dn_groups, max_dn_gt*2)
        ...
        for i in range(dn_anchor.shape[0]):
            cost = box_cost[i].cpu().numpy()  # (2 * max_dn_gt, max_dn_gt)
            anchor_idx, gt_idx = linear_sum_assignment(
                cost
            )  # (max_dn_gt,), (max_dn_gt, )
            ...
            if gt_track_id is not None:
                dn_id_target[i, anchor_idx] = gt_track_id[i, gt_idx]
        dn_anchor = (
            dn_anchor.reshape(self.num_dn_groups, bs, num_gt, state_dims)
            .permute(1, 0, 2, 3)
            .flatten(1, 2)
        )  # (bs, num_dn_groups*num_gt, 10)
        dn_box_target = (
    ...
    def update_dn(
        ...
        """
        Args:
            instance_feature: (bs,1220, 256)
            anchor: (bs,1220, 11)
            dn_reg_target: (bs, 320, 10)
            dn_cls_target: (bs, 320)
            valid_mask: (bs, 320)
            dn_id_target: (bs, 320)
            num_noraml_anchor: int 320
            temporal_valid_mask: None
        Return:
            dn_reg_target: (bs,1220, 10)
            dn_cls_target: (bs, 320)
        ...
        # update temp_dn_metas by track_id
    ...
    def cache_dn(
        ...
        Args:
            dn_instance_feature: (1, 320, 256)
            dn_anchor: (1, 320, 11)
            dn_cls_target: (1, 320)
            valid_mask: (1, 320)
            dn_id_target: (1, 320)
        Return:
            dn_instance_feature: (1, 3, 64, 256)
            dn_anchor: (1, 3, 64, 11))
            dn_cls_target: (1, 3, 64)
            valid_mask: (1, 3, 64)
            dn_id_target: (1, 3, 64)
        ...
        if self.num_temp_dn_groups < 0:  # default 3
            return
        num_dn_groups = self.num_dn_groups  # 5
