...
from dataset import NuScenes4DDetTrackVADDataset
...
class VectorEvaluate(object):
    ...
    def __init__(self, dataset_cfg, n_workers: int=N_WORKERS) -> None:
        self.dataset = build_module(dataset_cfg)
        ...
        self.id2cat = {str(v): k for k, v in self.cat2id.items()}
    ...
    def gts(self) -> Dict[str, Dict[int, List[NDArray]]]:
        ...
        for data in self.dataloader:
            token = deepcopy(data['img_metas'].data[0][0]['sample_idx'])
            gt = deepcopy(data['map_annos'])
            gts[token] = gt
            pbar.update()
